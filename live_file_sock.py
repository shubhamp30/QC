...
async def process_files(case_files):
    async with aiohttp.ClientSession() as session:
        ...
        downloaded_files = asyncio.gather(*download_tasks)
        executor = ThreadPoolExecutor(max_workers=4)
        split_tasks = [executor.submit(split_pdf_with_fitz, df) for df in downloaded_files if df[1] is not None]
        split_results = asyncio.gather(*[asyncio.wrap_future(task) for task in split_tasks])
...
async def process_all_files(split_files_dict):
    ...
    for case_id, files in split_files_dict.items():
        ...
        case_results =  asyncio.gather(*tasks)
...
@sock.route('/echo')
def quality_check(sock):
    try:
        sock.send(f'Processing Started .. ')
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if resp:
                ...
                split_results = process_files(case_files)
                ocr_results = process_all_files(split_results)
